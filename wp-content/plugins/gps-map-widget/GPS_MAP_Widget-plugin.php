...
class GPS_MAP_Widget extends WP_Widget {
	...
	function form($instance) {
	    ...
	    <p>
			...
			<p>
	    	<label for="<?php echo $this->get_field_id('apikey'); ?>"><?php _e('API key', self::FS_TEXTDOMAIN); ?></label>
...
// ------------------------------------------------------ Shortcodes
	...
	function custom_EXIF_locationmap( $atts ) {
		...
		if ($post_thumbnail_id) {
			$location = $this->getLocationFromDBorExif($post_thumbnail_id);
			$addKey = $atts['apikey']?'&key='.$atts['apikey']:'';
			if ($location['hasLocation']) {
				$res = "\n<!-- EXIF locationmap -->\n";
				...
				$res .=	'<img src="https://maps.googleapis.com/maps/api/staticmap?zoom='.$zoom.'&size='.$mapsize.'&markers=size:mid|'.$location['latitude'].','.$location['longitude'].$addKey.'" style="width:100%">';
				$res .= '</a>';
			} else {
					if ($errors) {
						$res = '<p style="text-align:center">'.__('There is no GPS information available',self::FS_TEXTDOMAIN).'</p>';
					}
			}
		} else {
			if ($errors) {
				$res ='<p style="text-align: center">'.__('There is no featured image available',self::FS_TEXTDOMAIN).'</p>';
			}
		}
...
// ------------------------------------------------------ Editor code attachment screen
	...
	function gps_map_box( $object, $box ) {
		...
		$zoom = '11';
		$mapsize = $width.'x'.$height;
		$addKey = $this->staticMapsApiKey?'&key='.$this->staticMapsApiKey:'';
		if ($location['hasLocation']) {
			$res = "\n<!-- GPS mapbox -->\n";
...
// ------------------------------------------------------ Editor code attachment screen
	...
	function add_header_code () {
		wp_enqueue_script('GPS_MAP_Widget_js_handler', plugins_url('/basics/basic_plugin.js', __FILE__ ), array( 'jquery' ));
	}
}
?>
